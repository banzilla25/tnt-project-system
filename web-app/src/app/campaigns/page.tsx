...
import { Trash2, Archive, CheckCircle2, Activity, Lock } from "lucide-react";
...
function CampaignCardItem({ campaign, brand, isManager }: { campaign: any, brand: any, isManager: boolean }) {
  ...
  return (
    <Card className={`hover:border-blue-500 transition-colors h-full flex flex-col relative overflow-hidden ${!hasAccess ? 'opacity-80 grayscale-[20%]' : ''}`}>
      <Link href={`/campaigns/${campaign.id}/listing`} className="block flex-1 cursor-pointer">
        <CardContent className="p-5 flex flex-col h-full relative">
          {!hasAccess && (
            <div className="absolute top-2 right-2 flex items-center gap-1 bg-slate-100 text-slate-500 text-[10px] px-2 py-1 rounded-full font-medium">
          ...
          <Badge variant={campaign.tipe_campaign === 'sales' ? 'success' : campaign.tipe_campaign === 'gmv_awareness' ? 'warning' : 'default'} className="uppercase text-[10px] w-fit mb-4">
      ...
      {isManager && (
        <div className="border-t border-slate-100 bg-slate-50 mt-auto">
          ...
          >
            {isExpanded ? "Tutup Pengaturan" : "Buka Pengaturan Target & Plafon"}
