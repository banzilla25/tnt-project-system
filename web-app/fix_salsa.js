...
async function run() {
  const { data: campaigns } = await supabase.from('campaigns').select('id, nama').in('nama', ['Salsa Cosmetics', 'Salsa Mom & Baby']);
  if (!campaigns || campaigns.length === 0) return;

  const workbook = xlsx.readFile('../TNT Project Tracking (Internal).xlsx');

  const nameMapping = {
    'Salsa Cosmetics': 'SALSA COSME',
    'Salsa Mom & Baby': 'SALSA M&B'
  };

  for (const campaign of campaigns) {
    let targetSheet = nameMapping[campaign.nama];
  ...
  console.log("Salsa campaigns processed!");
  process.exit(0);
}
