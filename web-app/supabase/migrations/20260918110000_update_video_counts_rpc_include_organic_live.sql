...
BEGIN
  ...
  WITH all_videos AS (
    ...
    -- 3. Live & Video dari organic_videos (data awareness live & video)
    SELECT 
      ov.content_uid,
      COALESCE(cc.approval, 'approved') as vt_approval,
      CASE 
        WHEN LOWER(ov.content_type) IN ('livestream', 'live', 'live stream') THEN 'livestream'
        ELSE 'video'
      END as content_type
    FROM organic_videos ov
    LEFT JOIN creators c ON LOWER(c.username) = LOWER(ov.creator_username)
    LEFT JOIN campaign_creators cc ON cc.creator_id = c.id AND cc.campaign_id = ov.campaign_id
    WHERE ov.campaign_id = p_campaign_id
      AND ov.content_uid IS NOT NULL
      AND ov.content_uid != ''
  ),
  deduped AS (
