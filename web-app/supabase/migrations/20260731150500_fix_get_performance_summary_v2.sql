  ...
  latest_ads AS (
    ...
    WHERE campaign_id = p_campaign_id
      AND (p_filter_type IS NULL 
           OR (p_filter_type = 'exclude' AND NOT (creator_id IN (SELECT c.id FROM campaign_creators cc LEFT JOIN creators c ON c.id = cc.creator_id WHERE c.username = ANY(p_filter_values))))
