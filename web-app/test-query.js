...
async function run() {
  const { data, error } = await supabase.from('payment_batches').select(`
    *,
    ...
    campaigns(nama),
    payment_items(
      id, nominal, biaya_transfer, final_status, payment_type, campaign_creator_id, metode_pembayaran, nomor_rekening, nama_penerima, notes_dari_pic, ratecard_awal, actual_transfer, executive_note, manager_note,
